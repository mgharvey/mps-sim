...
def check_biallelic(outdir, seq_length, outgroup):
	bsnps = 0
	poly = 0
	f1 = open("{0}tmp/seq-gen_output_reordered".format(outdir), 'r')
	alignment = list()
	for line in f1:
		parts = line.split()
		seq = parts[1]
		new_seq = seq.upper()
		alignment.append(list(new_seq))
	...
	for i in range(int(seq_length)):
		if outgroup == "True":
			...
			bases = alignment[i]		
		uniqs = set(bases).intersection('ACGT')		
		if len(uniqs) > 1:
			poly += 1
...
def write_nexus(outdir, l, ntax, seq_length, p, pop_sizes):				
	...
	nf = open("{0}/nex/locus{1}.nex".format(outdir, l), 'wb')				
	nf.write("#NEXUS\n")
	nf.write("begin taxa;\n")
...
def extract_SNPs(dataset_dir, outgroup):			
	...
	return all_samples, cat_align, cat_all_align


def write_summary(dataset_dir, ms, genes, seq_length, outgroup, poly, par, inv):
	...
	if outgroup == "True":
		...
		sf.write("number of loci filtered out due to too many polymorphisms (not including outgroup): {0}\n".format(par))
		sf.write("number of loci filtered because they were invariant (not including outgroup): {0}\n".format(inv))
	elif outgroup == "False":
...
def main():
	...
	print "Outgroup: {0}\n".format(outgroup)	
	snp_max = int(ConfigSectionMap("Arguments", Config)['snp_max'])
	print "Maximum number of SNPs per locus: {0}\n".format(snp_max)
	...
	for i in range(datasets): 
		...
		while j < genes: # until reach desired number loci
			...
			# check biallelic
			cb = check_biallelic(dataset_dir, seq_length, outgroup)
			poly += int(cb[0])
			bsnps = int(cb[1])
			if bsnps > int(snp_max):
				par += 1 # Loci filtered out due to too many SNPs
			elif bsnps == 0:
				inv += 1 # Loci filtered out because they are invariant
			elif bsnps > 0 and bsnps <= int(snp_max): # Loci within threshold for number of SNPs
				j += 1
				# write nexus alignments
				if nexus == "True": 
					write_nexus(dataset_dir, j, ntax, seq_length, p, pop_sizes)
				sys.stdout.write("\rGBS loci simulated: {0}".format((int(j))))
				sys.stdout.flush()		
			FNULL.close()
		if gphocs == "True": 
		...
		if nexus or summary or hapmap or dadi == "True":
			...
			cat_all_align = es[2]
			if nexus_all == "True": 
				print "\nWriting concatenated Nexus alignments..."
			...
			if nexus_one == "True": 
				if snp_max > 1:
					...
					print "\nWARNING: Each locus contains only one SNP. Printing only one alignment of concatenated SNPs."				
			if hapmap == "True": 
				print "\nWriting HapMap file..."
				biall = write_hapmap(dataset_dir, samples, cat_all_align)
				# default is to write hapmap with only single SNP per locus
			if summary == "True": 
				print "\nWriting summary statistics..."
				write_summary(dataset_dir, ms, genes, seq_length, outgroup, poly, par, inv)
			if dadi == "True": 
